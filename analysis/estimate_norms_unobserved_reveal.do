...
* this file produces norms_results_unobserved_reveal.dta which is then used to plot the norms/likelihood surfaces
* author: @lrdegeest
*===============================================================================
...
capture erase norms_results_unobserved_reveal.dta
postutil clear
tempname results
postfile `results' /// collect 10 estimation results
	...
	using norms_results_unobserved_reveal
	...
	foreach j in `sender_type' {
		forvalues k = 11/30 {
			...
			di "Extensive margin done."
			// intensive margin
			capture noisily qui xtpoisson target_sanction `controls' `dev_controls' if sender_type == `j' & target_sanction > 0, re vce(cluster groupid) iter(1000)
			post `results' (`k')  (`j') (2) (e(ll)) (_b[_cons]) (_b[target_cont]) (_b[mean_contribute]) (_b[dev]) (.) (e(N))
			di "Intensive margin done."
			drop dev
			di "DONE."
			di " "
		}
...
use norms_results_unobserved_reveal, clear 
* normalize
...
// save
save norms_results_unobserved_reveal, replace
*===============================================================================
